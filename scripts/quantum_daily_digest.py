#!/usr/bin/env python3
"""
Daily Quantum News: top N articles + LinkedIn post proposals.
Prioritises partnerships & large announcements; no repetition (14-day sent log).
"""
...
SENDER_ENV = "QUANTUM_DIGEST_SENDER"
DIGEST_NAME_ENV = "DIGEST_NAME"
DIGEST_FEEDS_ENV = "DIGEST_FEEDS"
DIGEST_TOPIC_KEYWORDS_ENV = "DIGEST_TOPIC_KEYWORDS"
DIGEST_TOP_N_ENV = "DIGEST_TOP_N"
...
<body style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 1em;">
  ...
  <p style="color: #555;">Drafts in your voice. Prioritised: IBM &amp; Red Hat–friendly news.</p>
  {"".join(linkedin_section)}

  <p style="margin-top: 1.5em; font-size: 0.85em; color: #888;">{title}. Daily Quantum News.</p>
</body>
</html>
"""
...
def build_email_plain(articles):
    date_str = datetime.now().strftime("%A, %B %d, %Y")
    title = _digest_title()
    lines = [f"{title} — {date_str}", "", f"TOP {len(articles)} ARTICLES", "---------------"]
    for i, a in enumerate(articles, 1):
